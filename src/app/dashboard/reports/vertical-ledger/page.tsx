...
async function fetchReport(type: string, params: Record<string, string>): Promise<ReportResult> {
  ...
  const res = await fetch(`/api/reports/vertical-ledger?${qs.toString()}`);
...
// Last day of a YYYY-MM month as a local date string. Not via toISOString():
// that converts to UTC first, so east of Greenwich the month-end midnight
// lands on the previous day and drops the last day of every month.
...
export default function VerticalLedgerPage() {
  ...
  const { data: report, isLoading, isError } = useQuery({
    queryKey: ['vertical-ledger', type, params],
    queryFn: () => fetchReport(type, params),
  });
  const { data: verticals = [] } = useQuery({ queryKey: ['verticals-for-vertical-ledger'], queryFn: fetchVerticals });

  useEffect(() => { if (isError) toast.error('Failed to load ledger'); }, [isError]);

  const exportPdf = () => {
    if (!report) return;
    generateReportPDF(report.title, report.columns, report.rows, `vertical-ledger-${type}-${new Date().toISOString().slice(0, 10)}.pdf`, buildPdfFoot(report));
  };

  // A vertical row covers many projects, so the popup scopes by project only
  // where the row is one project; otherwise it scopes by period and limits to
  // project-tagged spend — which is exactly what the ledger counted.
  const detailFor = (row: Record<string, any>, colKey: 'billed' | 'credit' | 'debit'): LedgerDetailTarget => {
    ...
    return {
      ...
      title: String(row.project ?? row.vertical ?? 'Selection'),
      ...
      expenseHref: colKey === 'debit' ? `/dashboard/reports/expenses?${qs.toString()}` : undefined,
    };
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 print:hidden">
        <div>
          ...
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-800">Vertical Ledger</h1>
            <p className="text-slate-500 text-xs">Credit and debit per business vertical, rolled up from its projects</p>
          </div>
        </div>
      ...
      <p className="text-xs text-slate-500 -mt-3">
        Overall Expenses carry no project, so no vertical — a vertical&apos;s Debit is its direct project cost, not its fully-loaded cost.
      </p>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-wrap gap-3 items-center print:hidden">
        ...
        {(verticalId || from || to) && (
          <button onClick={() => { setVerticalId(''); setFrom(''); setTo(''); }} className="text-sm text-slate-500 hover:text-red-500">Clear</button>
        )}
      </div>
                    ...
                    {report.columns.map((col) => (
                      <td key={col.key} className={`px-4 py-3 text-slate-700 ${col.align === 'right' ? 'text-right tabular-nums' : 'text-left'}`}>
                        {['billed', 'credit', 'debit'].includes(col.key) && Number(row[col.key]) > 0 ? (
                          <button type="button"
                            onClick={() => setDetail(detailFor(row, col.key as 'billed' | 'credit' | 'debit'))}
