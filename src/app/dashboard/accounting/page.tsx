...
export default function AccountingDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* KPI Cards — one full row per currency present in the data. In the
          common case (a single active currency) this renders exactly one
          row, same as before. */}
      {kpisByCurrency.length === 0 ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <KpiCard label="Total Invoices" value="0" color="text-slate-800" />
        </div>
      ) : (
        kpisByCurrency.map((kpis) => (
          <div key={kpis.currencyCode} className="space-y-2">
            {kpisByCurrency.length > 1 && (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <KpiCard label="Total Invoice Value" value={fmt(kpis.totalInvoiceValue ?? 0, kpis.currencyCode)} color="text-slate-800" />
              <KpiCard label="Amount Received" value={fmt(kpis.amountReceived ?? 0, kpis.currencyCode)} color="text-emerald-600" />
              <KpiCard label="Outstanding Amount" value={fmt(kpis.outstandingAmount ?? 0, kpis.currencyCode)} color="text-blue-600" />
              <KpiCard label="Overdue Amount" value={fmt(kpis.overdueAmount ?? 0, kpis.currencyCode)} color="text-red-600" />
              <KpiCard label="Due Today" value={fmt(kpis.dueToday ?? 0, kpis.currencyCode)} color="text-amber-600" />
              <KpiCard label="Due This Week" value={fmt(kpis.dueThisWeek ?? 0, kpis.currencyCode)} color="text-amber-600" />
