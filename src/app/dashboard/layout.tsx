...
async function fetchHeaderNotifications(): Promise<{ content: HeaderNotification[]; unreadCount: number }> {
  const res = await fetch('/api/notifications?size=6');
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Polls rather than pushing a live update on every new Notification row —
  // there's no websocket/SSE channel in this app, and 30s is frequent
  // enough for a header badge without hammering the API. Only runs once
  // there's a session (skip the anonymous login screen).
  ...
  const recentNotifications = notifData?.content ?? [];
  ...
  const markNotifRead = useMutation({
    ...
    // Also invalidates the full Notifications page's own query key, so
    // marking read from the header dropdown doesn't leave that page (if
    // open in another tab/on next visit) showing a stale unread badge.
    onSuccess: () => {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-14 bg-white border-b border-slate-200 flex items-center justify-between md:justify-end px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative" ref={notifMenuRef}>
            ...
            {notifOpen && (
              <div className="absolute right-0 mt-2 w-80 max-w-[90vw] bg-white rounded-lg shadow-lg border border-slate-200 z-50 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-100">
                  <p className="text-sm font-semibold text-slate-800">Notifications</p>
                  <Link href="/dashboard/notifications" onClick={() => setNotifOpen(false)} className="text-xs font-medium text-amber-700 hover:text-amber-800">
                    View all
                  </Link>
                </div>
                  ...
                  {recentNotifications.length === 0 ? (
                    ...
                    recentNotifications.map((n) => (
                      ...
                      >
                        ...
                        <p className="text-[11px] text-slate-400 mt-1">{dayjs(n.createdAt).fromNow()}</p>
                      </button>
                    ))
                  )}
                </div>
                <div className="px-4 py-2.5 border-t border-slate-100 bg-slate-50/60">
                  <PushNotificationToggle
                    hint="Get a browser notification the moment something new comes in, even when this tab isn't open"
                    enabledLabel="Browser notifications on"
                    disabledLabel="Enable browser notifications"
                    enabledToast="Browser notifications enabled"
                    disabledToast="Browser notifications turned off"
                    className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-slate-300 text-slate-600 hover:bg-white disabled:opacity-60"
                  />
                </div>
              </div>
            )}
