...
export default function PackagesPage() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (!res.ok) { const err = await res.json(); throw new Error(err.message || 'Failed to save package'); }
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['packages-admin'] }); queryClient.invalidateQueries({ queryKey: ['packages'] }); toast.success(editingId ? 'Package updated' : 'Package created'); closeForm(); },
  ...
  const toggleActive = useMutation({
    mutationFn: async ({ id, isActive }: { id: number; isActive: boolean }) => {
      const res = isActive
        ? await fetch(`/api/packages/${id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive: true }) })
        : await fetch(`/api/packages/${id}`, { method: 'DELETE' });
      if (!res.ok) { const err = await res.json(); throw new Error(err.message || 'Failed to update package'); }
      return res.json();
    },
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['packages-admin'] });
      queryClient.invalidateQueries({ queryKey: ['packages'] });
      toast.success(variables.isActive ? 'Package reactivated' : 'Package deleted');
    },
    onError: (err: Error) => toast.error(err.message),
  });
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-slate-900">
                <tr>
                  <th className="px-4 py-3 text-left font-semibold text-white">Package</th>
