...
import { ArrowLeftIcon, PlusIcon, ChevronDownIcon, ChevronUpIcon, TrashIcon, DocumentIcon } from '@heroicons/react/24/outline';
...
async function fetchDocuments(companyId: string, entityId: number): Promise<DocumentRow[]> {
...
function EntityDocuments({ companyId, entity }: { companyId: string; entity: LegalEntity }) {
...
export default function CompanyDetailPage() {
  ...
  const { data: company, isLoading } = useQuery({ queryKey: ['company', params.id], queryFn: () => fetchCompany(params.id) });

  const [companyForm, setCompanyForm] = useState({ name: '', notes: '' });
  useEffect(() => { if (company) setCompanyForm({ name: company.name, notes: company.notes || '' }); }, [company]);

  const saveCompany = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/companies/${params.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(companyForm) });
      if (!res.ok) { const err = await res.json(); throw new Error(err.message || 'Failed to update company'); }
      return res.json();
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['company', params.id] }); queryClient.invalidateQueries({ queryKey: ['companies'] }); toast.success('Company updated'); },
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  const saveEntity = useMutation({
    mutationFn: async () => {
      const url = editingEntityId ? `/api/companies/${params.id}/legal-entities/${editingEntityId}` : `/api/companies/${params.id}/legal-entities`;
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['company', params.id] }); toast.success(editingEntityId ? 'Legal entity updated' : 'Legal entity added'); closeEntityForm(); },
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  const deactivateEntity = useMutation({
    mutationFn: async (entity: LegalEntity) => {
      if (entity._count.leads + entity._count.quotations > 0) {
        const res = await fetch(`/api/companies/${params.id}/legal-entities/${entity.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive: false }) });
        if (!res.ok) { const err = await res.json(); throw new Error(err.message || 'Failed to deactivate legal entity'); }
        return res.json();
      }
      const res = await fetch(`/api/companies/${params.id}/legal-entities/${entity.id}`, { method: 'DELETE' });
      if (!res.ok) { const err = await res.json(); throw new Error(err.message || 'Failed to delete legal entity'); }
      return null;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['company', params.id] }); toast.success('Legal entity removed'); },
    onError: (err: Error) => toast.error(err.message),
  });
  ...
  return (
    <div className="space-y-4">
      <button onClick={() => router.push('/dashboard/companies')} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-800">
        <ArrowLeftIcon className="h-4 w-4" /> Back to Companies
      </button>

      <form
        onSubmit={(e) => { e.preventDefault(); if (!companyForm.name.trim()) { toast.error('Company name is required'); return; } saveCompany.mutate(); }}
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${company.isActive ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>{company.isActive ? 'Active' : 'Inactive'}</span>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Company Name</label>
            <input value={companyForm.name} onChange={(e) => setCompanyForm((f) => ({ ...f, name: e.target.value }))} className={inputCls} />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Notes</label>
            <input value={companyForm.notes} onChange={(e) => setCompanyForm((f) => ({ ...f, notes: e.target.value }))} className={inputCls} />
          </div>
        </div>
        <div className="flex justify-between items-center mt-3">
          <p className="text-xs text-slate-400">{company._count.leads} lead(s)/customer(s) linked to this company</p>
          <button type="submit" disabled={saveCompany.isPending} className="px-4 py-2 bg-amber-600 text-white text-sm font-medium rounded-lg hover:bg-amber-700 disabled:opacity-50">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 sm:p-5">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-base font-semibold text-slate-800">Legal Entities</h2>
          <button
        ...
        {showEntityForm && (
          ...
          >
            <h3 className="text-sm font-semibold text-slate-700">{editingEntityId ? 'Edit Legal Entity' : 'New Legal Entity'}</h3>
              ...
              <div key={e.id} className="py-3">
                ...
                {expandedId === e.id && (
                  <div className="mt-3 pl-1">
                    ...
                    <EntityDocuments companyId={params.id} entity={e} />
                  </div>
                )}
              </div>
            ))}
