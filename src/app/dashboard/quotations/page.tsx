...
export default function QuotationsPage() {
  ...
  const saveQuotation = () => {
    ...
    const newQ = {
      id: Date.now(), quotationNumber: 'QTN-' + dayjs().format('YYYYMMDD-HHmm'),
      clientName, companyName, modules: [...selectedModules, ...customModules.filter(c => c.name).map(c => c.name)],
      country: clientCountry, currency: pricing.currencyCode, totalAmount: pricing.grandTotal + customModulesTotal,
    ...
    toast.success(`Quotation ${newQ.quotationNumber} created!`);
    setView('list');
  };

  const downloadPDF = () => {
    if (!pricing) return;
    let lines = `QUOTATION: QTN-${dayjs().format('YYYYMMDD')}\nClient: ${clientName}\nCompany: ${companyName}\n\nModules:\n`;
    pricing.modules.forEach(m => { lines += `  ${m.moduleName}: ${pricing.currencySymbol}${m.basePrice.toLocaleString()}\n`; });
    customModules.filter(c => c.name && c.cost > 0).forEach(c => { lines += `  ${c.name} (Custom): ${pricing.currencySymbol}${(c.cost * c.quantity).toLocaleString()}\n`; });
    lines += `\nSubtotal: ${pricing.currencySymbol}${(pricing.subtotal + customModulesTotal).toLocaleString()}`;
    if (pricing.discountAmount > 0) lines += `\nDiscount (${pricing.discountPercentage}%): -${pricing.currencySymbol}${pricing.discountAmount.toLocaleString()}`;
    lines += `\n\nTaxes:\n`;
    pricing.taxBreakdown.forEach(t => { lines += `  ${t.taxName} (${t.rate}%): ${pricing.currencySymbol}${t.amount.toLocaleString()}\n`; });
    lines += `\nGRAND TOTAL: ${pricing.currencySymbol}${(pricing.grandTotal + customModulesTotal).toLocaleString()}\n\n---\nGenerated by Tekfilo CRM`;
    const blob = new Blob([lines], { type: 'text/plain' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `Quotation_${companyName || 'Client'}_${dayjs().format('YYYYMMDD')}.txt`; a.click();
            ...
            <tbody className="divide-y divide-slate-100">
              {quotations.map((q: any) => (
                <tr key={q.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3"><p className="font-medium">{q.clientName}</p><p className="text-xs text-slate-500">{q.companyName}</p></td>
                  <td className="px-4 py-3"><div className="flex flex-wrap gap-1">{q.modules.map((m: string) => <span key={m} className={`px-2 py-0.5 rounded text-xs font-medium ${MODULE_COLORS[m] || 'bg-orange-100 text-orange-700'}`}>{m.charAt(0) + m.slice(1).toLowerCase()}</span>)}</div></td>
                  <td className="px-4 py-3 text-right font-semibold">{q.currency} {q.totalAmount.toLocaleString()}</td>
                  <td className="px-4 py-3"><span className="px-2 py-1 rounded text-xs font-medium bg-slate-100">{q.status}</span></td>
                  <td className="px-4 py-3 text-slate-500">{dayjs(q.createdAt).format('DD MMM')}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Client Name *</label><input value={clientName} onChange={e => setClientName(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" /></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Company *</label><input value={companyName} onChange={e => setCompanyName(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" /></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Email</label><input type="email" value={clientEmail} onChange={e => setClientEmail(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" /></div>
              <div><label className="block text-sm font-medium text-slate-700 mb-1">Phone</label><input value={clientPhone} onChange={e => setClientPhone(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500" /></div>
            </div>
          </div>
            ...
            {customModules.length > 0 && (
              <div className="mt-4 space-y-3">{customModules.map(cm => (
                <div key={cm.id} className="p-4 rounded-lg border border-orange-200 bg-orange-50 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div><label className="text-xs font-medium text-slate-600">Name *</label><input value={cm.name} onChange={e => updateCustomModule(cm.id, 'name', e.target.value)} placeholder="AI Integration" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm mt-1" /></div>
                    <div className="flex gap-2"><div className="flex-1"><label className="text-xs font-medium text-slate-600">Cost (₹) *</label><input type="number" value={cm.cost} onChange={e => updateCustomModule(cm.id, 'cost', Number(e.target.value))} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm mt-1" /></div><div className="w-16"><label className="text-xs font-medium text-slate-600">Qty</label><input type="number" min={1} value={cm.quantity} onChange={e => updateCustomModule(cm.id, 'quantity', Number(e.target.value))} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm mt-1" /></div></div>
                  </div>
                  <div><label className="text-xs font-medium text-slate-600">Description *</label><textarea rows={2} value={cm.description} onChange={e => updateCustomModule(cm.id, 'description', e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm mt-1" /></div>
                </div>
