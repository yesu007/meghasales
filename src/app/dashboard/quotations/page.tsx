...
export default function QuotationsPage() {
  ...
  const downloadQuotationPDF = (q: any) => {
    ...
    if (snapshot?.modules?.length > 0) {
      lines += `\nPricing Breakdown:\n`;
      snapshot.modules.forEach((m: any) => { lines += `  ${m.moduleName}: ${snapshot.currencySymbol}${Number(m.basePrice).toLocaleString()}\n`; });
      if (snapshot.implementationCost > 0) lines += `  Implementation: ${snapshot.currencySymbol}${Number(snapshot.implementationCost).toLocaleString()}\n`;
      if (snapshot.trainingCost > 0) lines += `  Training: ${snapshot.currencySymbol}${Number(snapshot.trainingCost).toLocaleString()}\n`;
      if (snapshot.annualMaintenanceCost > 0) lines += `  AMC: ${snapshot.currencySymbol}${Number(snapshot.annualMaintenanceCost).toLocaleString()}\n`;
      if (snapshot.discountAmount > 0) lines += `  Discount (${snapshot.discountPercentage}%): -${snapshot.currencySymbol}${Number(snapshot.discountAmount).toLocaleString()}\n`;
      lines += `\nTaxes:\n`;
      (snapshot.taxBreakdown || []).forEach((t: any) => { lines += `  ${t.taxName} (${t.rate}%): ${snapshot.currencySymbol}${Number(t.amount).toLocaleString()}\n`; });
    }
    lines += `\nGRAND TOTAL: ${q.currencyCode} ${Number(q.totalAmount).toLocaleString()}\n\n---\nGenerated by Tekfilo CRM`;
    const blob = new Blob([lines], { type: 'text/plain' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `Quotation_${q.quotationNumber}_${q.companyName}.txt`;
    a.click();
    toast.success('Downloaded!');
  };
