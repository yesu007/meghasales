...
import { submitMom, OptimisticLockError, InvalidStatusTransitionError } from '@/lib/meetings/momService';
...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const mom = await submitMom(momId, Number(body.version), currentUserId(session));
    ...
    console.error('POST /api/mom/[id]/submit error:', error);
    return NextResponse.json({ message: error.message || 'Failed to submit MOM' }, { status: 400 });
  }
}
