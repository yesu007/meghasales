...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    // what's in this request body. Same rule as PATCH /api/projects/[id].
    ...
    // No duplicate Product — same check as POST /api/products, run against
    ...
    {
      const nextProductName = body.productName !== undefined ? String(body.productName).trim() : existing.productName;
      ...
      if (nextIsActive) {
        const duplicate = await prisma.product.findFirst({
          where: {
            id: { not: id },
            isActive: true,
            productName: { equals: nextProductName, mode: 'insensitive' },
    ...
    await logAudit({ action: 'UPDATE', entityType: 'PRODUCT', entityId: id, oldValue: existing, newValue: product, description: `Product "${product.productName}" for "${ownerName}" updated`, request });
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    await logAudit({ action: 'DELETE', entityType: 'PRODUCT', entityId: id, oldValue: existing, description: `Product "${existing.productName}" for "${ownerName}" deleted`, request });

    return NextResponse.json(product);
  } catch (error) {
    console.error('DELETE /api/products/[id] error:', error);
    return NextResponse.json({ message: 'Failed to delete product' }, { status: 400 });
  }
}
