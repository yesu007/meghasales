...
import { PlusIcon, ClockIcon, PhoneIcon } from '@heroicons/react/24/outline';
              ...
              <li key={f.id} className="flex gap-3">
                ...
                <div className="pb-1 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm font-medium text-slate-800">{followUpMethodLabel(f.method)}</span>
                    <span className="text-xs text-slate-400">{dayjs(f.followUpDate).format('DD MMM YYYY')}</span>
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    {f.loggedBy ? `Logged by ${f.loggedBy.firstName} ${f.loggedBy.lastName} · ` : ''}{dayjs(f.createdAt).format('DD MMM YYYY, HH:mm')}
                  </p>
                </div>
              </li>
            ))}
