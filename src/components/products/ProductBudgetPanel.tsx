...
// calculation and layout) — Product Master's own "Budget Estimation" panel,
// expanded under a Product row the same way Project's own panel expands
// under a Project row. The only difference is what each figure is scoped
// to: this reads Quotation.productId / Expense.productId instead of
// projectId. Edit/+ New Estimation still link to this app's *other*
// quotation form (/dashboard/quotations, catalog/module-based) instead of
// the Calculator — see those links' own comment for why.
...
// toward the product's planned spend — a Draft/Negotiation/Rejected/Expired
// quotation isn't a budget anyone has actually signed off on yet. Same rule
// as ProjectBudgetPanel.
const BUDGET_STATUSES = ['SENT', 'APPROVED'];

async function fetchProductQuotations(productId: number): Promise<QuotationRow[]> {
  const res = await fetch(`/api/quotations?productId=${productId}&size=100&sortBy=createdAt&sortDir=desc`);
  if (!res.ok) throw new Error('Failed to fetch budget estimations');
  const data = await res.json();
  return data.content;
}

async function fetchProductExpenses(productId: number): Promise<ExpenseRow[]> {
  const res = await fetch(`/api/expenses?productId=${productId}&size=500`);
...
export default function ProductBudgetPanel({ productId, newEstimationHref }: { productId: number; newEstimationHref?: string }) {
  ...
  const { data: expenses = [], isLoading: loadingExpenses } = useQuery({
    queryKey: ['product-budget-expenses', productId],
    queryFn: () => fetchProductExpenses(productId),
  });
  ...
  const deleteQuotation = async (id: number, quotationNumber: string) => {
    ...
    queryClient.invalidateQueries({ queryKey: ['product-budget-quotations', productId] });
  ...
  return (
    <div className="py-4 space-y-5">
      <div>
        ...
        {quotations.length === 0 ? (
          <p className="text-sm text-slate-400">No Budget Estimation created for this product yet.</p>
        ) : (
