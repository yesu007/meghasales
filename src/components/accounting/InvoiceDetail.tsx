...
// The invoice detail view itself — no route, no URL knowledge. Both the
// Accounting module's own route (src/app/dashboard/accounting/invoices/[id]/page.tsx)
// and the Customer module's nested route
// (src/app/dashboard/customers/[id]/invoices/[invoiceId]/page.tsx) render
// this same component and just pass the `backHref` appropriate to where the
// viewer came from — no query params, no shared/global navigation state, so
  ...
  if (!invoice) {
    return (
      <div className="text-center py-16">
        ...
        <Link href={backHref} className="text-amber-600 hover:text-amber-700 text-sm mt-2 inline-block">← Back to Invoices</Link>
      </div>
    );
  }
