...
async function buildAgingReport(filters: ReportFilters): Promise<ReportResult> {
  ...
  const byCustomer: Record<string, { current: number; d31_60: number; d61_90: number; d90plus: number }> = {};
  for (const inv of invoices) {
    const name = inv.lead.companyName;
    if (!byCustomer[name]) byCustomer[name] = { current: 0, d31_60: 0, d61_90: 0, d90plus: 0 };
    const daysOverdue = Math.max(0, today.diff(dayjs(inv.dueDate), 'day'));
    const balance = Number(inv.balanceDue);
    if (daysOverdue <= 30) byCustomer[name].current += balance;
    else if (daysOverdue <= 60) byCustomer[name].d31_60 += balance;
    else if (daysOverdue <= 90) byCustomer[name].d61_90 += balance;
    else byCustomer[name].d90plus += balance;
  }
  ...
  return {
    ...
    rows: Object.entries(byCustomer).map(([companyName, b]) => ({
      companyName,
      current: b.current,
      d31_60: b.d31_60,
      d61_90: b.d61_90,
...
async function buildPaymentHistoryReport(filters: ReportFilters): Promise<ReportResult> {
  const payments = await prisma.payment.findMany({
    ...
    include: { invoice: { select: { invoiceNumber: true, lead: { select: { companyName: true } } } } },
...
async function buildCollectionReport(filters: ReportFilters): Promise<ReportResult> {
  ...
  return {
    ...
    rows: Object.entries(byCustomer)
      .map(([companyName, b]) => ({ companyName, count: b.count, total: b.total }))
...
async function buildMonthlyCollectionReport(filters: ReportFilters): Promise<ReportResult> {
  ...
  for (let i = numMonths - 1; i >= 0; i--) months[dayjs().subtract(i, 'month').format('MMM YYYY')] = { count: 0, total: 0 };
  for (const p of payments) {
    const key = dayjs(p.paymentDate).format('MMM YYYY');
    if (key in months) { months[key].count += 1; months[key].total += Number(p.amount); }
  }
  ...
  return {
    ...
    rows: Object.entries(months).map(([month, b]) => ({ month, count: b.count, total: b.total })),
  };
}
